...
struct monitored_entry {
    ...
    int soft_warned;          /* 1 after soft-limit warning emitted */
 ...
 *
 * We use a mutex (not spinlock) because the timer callback and ioctl
 * both run in sleepable contexts on this kernel version, and mutex
 * gives cleaner semantics for list iteration with possible alloc/free.
 * ---------------------------------------------------------------- */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * list_for_each_entry_safe lets us safely delete entries while
     * iterating, avoiding use-after-free.
     */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        /* TODO 4: Allocate and insert new entry */
